...
class Topup {
    ...
    function topupStepOne($ussd_body, $mobile_number, $session_id, $sessdetails) {
        try {
            ...
            $data['mno_display_list'] = $subminimenu['menulist'];
            $data['mno_code_list'] = implode("|", $subminimenu['subminicode']);
            ...
            $data['status'] = 'topup|topupStepTwo|Topup';
     ...
     * &&
     * Ask user to select MNO e.g safaricom
     */

    function topupStepTwo($ussd_body, $mobile_number, $session_id, $sessdetails) {
        try {
            ...
            if ($account_selected == 0 || $account_selected > $number_of_accounts) {
                $responseArr['USSD_BODY'] = $sessdetails['lang']['invalid_selection'] . $sessdetails['previous_responseprevious_response'];
                ...
                $responseArr['USSD_BODY'] = $sessdetails['mno_display_list'] . $sessdetails['lang']['home_or_back'];

                $data['accfrom'] = $available_accounts[$account_selected - 1];
                $data['menu'] = 'Menu';
                $data['status'] = 'topup|topupStepThree|Topup';
    ...
    /* Check which MNO the user wants to buy airtime for
     * &&
     * Ask user to select number to buy airtime for
     */

    function topupStepThree($ussd_body, $mobile_number, $session_id, $sessdetails) {
        try {
            $mno_selected = intval(trim($ussd_body));
            $available_mnos = explode("|", $sessdetails['mno_code_list']);
            $number_of_mnos = count($available_mnos);

            $responseArr['END_OF_SESSION'] = "False";
            //Check if user wants to proceed or go home/back
            if ($mno_selected == 99 || $mno_selected == 77) {
                $responseArr = $this->functions->checkNextStep($ussd_body, $mobile_number, $session_id, $sessdetails);
                return $responseArr;
            }

            if ($mno_selected == 0 || $mno_selected > $number_of_mnos) {
                $responseArr['USSD_BODY'] = $sessdetails['lang']['invalid_selection'] . $sessdetails['previous_response'];
                $this->functions->flog('INFO', __METHOD__ . "() | Either the selected value was not an integer or the MNO selected was not on list of MNOs on the screen ");
            } else {
                $responseArr['USSD_BODY'] = $sessdetails['lang']['number_to_topup_menu'] . $sessdetails['lang']['home_or_back'];

                $data['mno_selected'] = $available_mnos[$mno_selected - 1];
                $data['menu'] = 'Menu';
                $data['status'] = 'enteramount|topupStepFour|Topup';
                $data['previous_response'] = $responseArr['USSD_BODY'];
                $this->functions->updateSession($mobile_number, $data);
            }
            return $responseArr;
    ...
    function topupStepFour($ussd_body, $mobile_number, $session_id, $sessdetails) {
        try {
            ...
            if ($user_selection == 99 || $user_selection == 77) {
                ...
                switch ($user_selection) {
                    case 1: //Own number ~~  Ask for topup amount
                        $data['accto'] = $mobile_number;
                        $data['status'] = 'enteramount|topupStepSix|Topup';
                        $responseArr['USSD_BODY'] = $lang['enter_topup_amount'];
                        break;
                    case 2: //Other number ~~ Ask user to enter number
                        $data['status'] = 'enteramount|topupStepFive|Topup';
     ...
     * &&
     * Ask user to enter amount to topup
     */

    function topupStepFive($ussd_body, $mobile_number, $session_id, $sessdetails) {
        try {
            $responseArr['END_OF_SESSION'] = "false";
            ...
            if ($number_valid['status'] == true) {
                $responseArr['USSD_BODY'] = $sessdetails['lang']['enter_topup_amount']; //.$sessdetails['lang']['home_or_back'];;
    ...
    /* Check the topup amount entered by user. Min  is 10
     * &&
     * Ask user to confirm or cancel transaction
     */

    function topupStepSix($ussd_body, $mobile_number, $session_id, $sessdetails) {
        try {
...
//            }
            ...
            if ($amount_entered < 10 || strlen($amount_entered) == 0) {
                $responseArr['USSD_BODY'] = $sessdetails['lang']['topup_amount_invalid'] . $sessdetails['previous_response'];
                $this->functions->flog('INFO', __METHOD__ . $mobile_number . "() | Account number entered is either wrong or not a valid account number");
            } else {
                $responseArr['USSD_BODY'] = str_replace("[PHONE]", $sessdetails['accto'], str_replace("[ACCOUNT_FROM]", $sessdetails['accfrom'], str_replace("[MNO]", $sessdetails['mno_selected'], str_replace("[AMOUNT]", $amount_entered, $sessdetails['lang']['confirm_topup']))));
                ...
                $data['menu'] = 'Menu';
                $data['previous_response'] = $responseArr['USSD_BODY'];
                $data['status'] = 'enteramount|topupStepSeven|Topup';
                $this->functions->updateSession($mobile_number, $data);
            }
            return $responseArr;
        } catch (Exception $e) {
    ...
    function topupStepSeven($ussd_body, $mobile_number, $session_id, $sessdetails) {
        try {
            $responseArr['END_OF_SESSION'] = "false";
            $sessionStatus = "final|endTxn|Functions";

            if (intval($ussd_body) == 1) {//post the transaction
                $spdetails = $this->functions->getServiceProvider("AIRTIMETOPUP");
                ...
                $postdetails['fld90'] = "TOPUP";
                $postdetails['fld100'] = $sessdetails['mno_selected'];
                ...
                if ($result['data']['field39'] == '00' || $result['data']['field39'] == '00') {
					...
					//$this->functions->send_sms('+254726128484', 'Buda! airtime topup imebeat!');
                }
